...
- 当前卡牌设计：`docs/AK_Exusiai-Card_V1.2.csv`；旧版与平衡调整记录位于 `docs/archive/`。
- 遗物与药水设计：`docs/AK_Exusiai-Relic_Potion_V1.2.csv`。
- 当前状态：V1.2 发布准备阶段，正式卡图、六套外观、能量 UI、转场与卡牌音效已接入。内容统计、构建证据和待验项目只维护在进度页，不把“已有实现”写成“全部通过实战回归”。
...
- 共感形态在玩家回合开始时选择手牌给予天使，并在每次打出天使牌后抽牌；当前实现不额外重放卡牌。不要恢复旧版自动赋予/递归重放逻辑。
