...
- 设计表：`design/Exusiai_Cardlist-卡牌.csv`。双方在线确认设计后导出 CSV；实现时只读对照，不由代码任务覆盖或机械改写。`design/.gdignore` 用于阻止 Godot 把设计 CSV 误当作翻译资源。
...
当前设计以 `design/` 下的卡牌 CSV 为准；历史 Excel 已停止使用。核心机制至少包括：
