...
- 当前卡牌设计：`docs/AK_Exusiai-Card_V1.csv`；旧版与平衡调整记录位于 `docs/archive/`。
- 遗物与药水设计：`docs/AK_Exusiai-Relic_Potion_V1.csv`。
...
- 天使牌获得保留；被给予天使后第一次打出强制免费且不受其他费用效果影响。免费资格消耗后天使身份仍保留，再次给予天使会刷新资格。
- 状态或诅咒牌在任意战斗牌堆中获得天使后立即消耗，并正常触发告解等消耗响应。
- 共感形态只在消耗天使首次免费资格时额外打出，不因普通 0 费触发；能力牌也可触发。
- 身份、首次免费资格和重复给予状态必须保存并保持多人确定性；费用查询、描述刷新和悬停预览不得写模型状态。

### 干扰与沉默（V1）

- 干扰永久保留，每层使敌人造成的伤害降低 10%，最多降低 50%。
