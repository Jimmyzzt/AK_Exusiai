...
public sealed class GuardianGun : ExusiaiCardTemplate
{
    private const string ThresholdKey = "Threshold";
    protected override bool ShowAmmoHoverTip => true;
    protected override IEnumerable<DynamicVar> CanonicalVars => [new DynamicVar(ThresholdKey, 4m)];

    public GuardianGun() : base(1, CardType.Power, CardRarity.Rare, TargetType.Self)
    {
    }

    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        await PowerCmd.Apply<GuardianGunPower>(
            choiceContext,
            Owner.Creature,
