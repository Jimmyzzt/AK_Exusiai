...
- 激光炮？、过载模式、放逐：分别测试 0、1、上限、超过上限的弹药；每发弹药都强化每一段，实际耗弹不超过 5/5/6。再叠加临时改装与帕格尼尼定制验证总增伤。
...
- 爆弹：分别用单体、群体、多段攻击测试；原攻击照常命中，额外群伤按每段弹药增伤触发，并计入临时改装与帕格尼尼定制。轻装上阵、德克萨斯等完全未耗弹的免费攻击不触发；开火成瘾症预付过弹药的自动攻击触发。
...
- 使命必达！、物流外包、冰镇甜品！、帕格尼尼定制：回归快递选择、自动打出、升级费用/层数，以及多余能量与回合开始弹药。
