...
| 物料 | 建议规格 | 内容与当前状态 |
| --- | --- | --- |
| 工坊主封面 | 方形 1024×1024 工作图，导出 PNG 小于 1 MB，建议控制在 900 KB 内 | 能天使或双外观为主体，红/金/黑主色，保留光环、铳械、苹果元素；标题只留“能天使 / Exusiai”，小缩略图也可读。待制作 |
| 截图 1：角色选择 | 1920×1080，或保留原生 16:10 比例 | 默认能天使、角色介绍与初始遗物；采用当前版本实机图。待截取 |
...
| 截图 6：外观与合作 | 同上 | 新约外观或真实多人合作画面；六套外观拼图也可。待截取 |
...
素材已有基础：`AK_Exusiai/images/character/appearance/` 中的两张背景、现有六套外观、卡图和苹果能量图标均可用于设计参考。角色背景不等于已经做好的工坊封面。历史反馈截图有旧数值和报错，不宜直接用于首发宣传。
...
1. 审阅介绍、封面与实机截图；补齐 [素材记录](CREDITS.md) 中尚缺的来源；共同决定源码许可范围。
