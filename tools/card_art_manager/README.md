...
- “显示占位图形”是独立开关。选择素材会自动关闭它，清除素材关联会自动打开它；也可以手动组合背景、素材和占位图形三层。
...
清单版本 3 使用 `background_enabled`、`placeholder_enabled`、`rotation`、`flip_horizontal` 和 `flip_vertical` 字段。旧版 `mode` 配置会在首次打开时自动迁移；旧透明图标的缩放值会换算为统一倍率以尽量保持原有大小。
...
- 已加载原图缓存在内存中，关闭工具即释放。缩略图除内存缓存外，还保存在 `user://card_art_manager_thumbnails/`；Windows 上通常对应 `%APPDATA%\Godot\app_userdata\AK_Exusiai\card_art_manager_thumbnails\`。文件名包含源路径、修改时间和大小的摘要，源图变化后会自动使用新缓存；该目录可以安全删除并让工具重建。
